%% This study is to evaluate the weight of activity constraint 
...
global delta; 
global wActivity;wActivity=0.01;
deltas=[10 20 30 40 50];
nd=length(deltas);
pVals_d=zeros(nd,length(tp_w));
fVals_d=zeros(nd,1);
dscL_d=zeros(nd,1);
dmL_d=zeros(dimX,dimY,nd);
for k=1:nd
    delta=deltas(k);
    [pVals_d(k,:),fVals_d(k)] = fmincon(@objConFunc,initP_w,[],[],[],[],[],[],@noconstraint,options_w); 
    [dscL_d(k), dmL_d(:,:,k)] = calcDSC(pVals_d(k,:), tp_w);
end;

formats_d=['ro-';  'bo-'; 'go-'; 'mo-' ;'yo-'; 'co-'];
...
strLegend_d=cell(nd+1,1);
strLegend_d{1}='truth';
for k=1:nd
    actL_w=pVals_d(k,4+2*nRad+1:4+2*nRad+nSeg);
    plot(1:nSeg,actL_w,formats_d(k,:));
    strLegend_d{k+1}=sprintf('delta=%.1f',deltas(k));   
end
legend(strLegend_d);
hold off;
...
%     strLegend_w{k+1}=sprintf('Shape weight=%.3f',weights(k));   
% end
% legend(strLegend_w);
% hold off;
% 
